...
const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  // Dashboard states
  const [activeTab, setActiveTab] = useState<'inventory' | 'orders' | 'workshops' | 'product-orders' | 'artisans' | 'manage-artisans' | 'pending-products' | 'pending-workshops' | 'active-workshops' | 'moderation' | 'marketplace-requests' | 'institution-requests' | 'journal' | 'invoices' | 'shipping' | 'bookings' | 'media'>('inventory');
  ...
  const tabs: { id: string; label: string; count: number; icon: React.ReactNode }[] = [
    ...
    { id: 'media', label: 'Media Library', count: 0, icon: <svg className="w-[18px] h-[18px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="square" strokeLinejoin="miter" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg> },
  ...
  return (
    <div className="flex min-h-screen bg-[#FAF9F6]">
      ...
      {activeTab === 'media' && (() => {
        const allMedia = [
          ...products.flatMap(p => p.images.map((img, i) => ({ id: `${p.id}-img-${i}`, docId: p.id, type: 'Product', parentName: p.name, src: img, index: i }))),
          ...artisans.filter(a => a.profilePhoto).map(a => ({ id: `${a.id}-photo`, docId: a.id, type: 'Artisan', parentName: a.name, src: a.profilePhoto!, index: 0 })),
          ...workshops.filter(w => w.image).map(w => ({ id: `${w.id}-img`, docId: w.id, type: 'Workshop', parentName: w.title, src: w.image!, index: 0 })),
          ...journalEntries.filter(j => j.image).map(j => ({ id: `${j.id}-img`, docId: j.id, type: 'Journal', parentName: j.title || 'Journal Entry', src: j.image!, index: 0 }))
        ];

        const handleDeleteMedia = async (mediaItem: any) => {
          if (!window.confirm(`Delete this image from ${mediaItem.type}: ${mediaItem.parentName}?`)) return;
          try {
            if (mediaItem.type === 'Product') {
              const product = products.find(p => p.id === mediaItem.docId);
              if (product) {
                const newImages = [...product.images];
                newImages.splice(mediaItem.index, 1);
                await setDoc(doc(db, 'products', product.id), { ...product, images: newImages });
              }
            } else if (mediaItem.type === 'Artisan') {
              const artisan = artisans.find(a => a.id === mediaItem.docId);
              if (artisan) await setDoc(doc(db, 'artisans', artisan.id), { ...artisan, profilePhoto: '' });
            } else if (mediaItem.type === 'Workshop') {
              const workshop = workshops.find(w => w.id === mediaItem.docId);
              if (workshop) await setDoc(doc(db, 'workshops', workshop.id), { ...workshop, image: '' });
            } else if (mediaItem.type === 'Journal') {
              const journal = journalEntries.find(j => j.id === mediaItem.docId);
              if (journal) await setDoc(doc(db, 'journal', journal.id), { ...journal, image: '' });
            }
          } catch (err: any) {
        ...
        return (
          <div className="space-y-8 animate-in fade-in duration-500">
            <div>
              <h2 className="text-2xl serif text-[#2C2C2C] mb-2 font-bold">Media Library</h2>
              <p className="text-sm text-[#999] font-light">Manage all active photos distributed across products, artisans, workshops, and journals.</p>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
              {allMedia.map(item => (
                <div key={item.id} className="bg-white border border-[#E5E5E5] p-2 flex flex-col group relative">
                  <div className="w-full aspect-square bg-gray-100 overflow-hidden mb-2 relative">
                    <img src={item.src} alt={item.parentName} className="w-full h-full object-cover group-hover:scale-105 transition-transform" loading="lazy" />
                    <button 
                      onClick={() => handleDeleteMedia(item)}
                      className="absolute top-2 right-2 bg-white/90 text-red-600 rounded-full p-1.5 opacity-0 group-hover:opacity-100 hover:bg-red-50 hover:text-red-700 transition-all shadow-sm"
                      title="Delete Image"
                    >
                  ...
                  <div className="px-1 text-center">
                    <span className="text-[8px] uppercase tracking-widest text-[#8B735B] font-bold bg-[#FAF9F6] px-1.5 py-0.5 mb-1 inline-block border border-[#8B735B]/20">{item.type}</span>
                    <p className="text-[10px] text-[#2C2C2C] font-semibold truncate w-full">{item.parentName}</p>
                  </div>
                </div>
              ))}
              {allMedia.length === 0 && (
                <div className="col-span-full py-20 text-center text-[#999]">No media files found.</div>
              )}
            </div>
          </div>
        );
